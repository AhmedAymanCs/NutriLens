...
class HistoryCubit extends Cubit<HistoryState> {
  ...
  HistoryCubit(this._historyRepository)
    : super(
        HistoryState(selectedDate: DateTime.now(), focusedDay: DateTime.now()),
      );
  ...
  Future<void> fetchHistory(DateTime date) async {
    ...
    result.fold(
      (error) =>
          emit(state.copyWith(status: HistoryStatus.failure, error: error)),
      (data) {
  ...
  void _applyFilter(String filter) {
    ...
    final filteredData = _originalHistoryData!.copyWith(
      todayMeals: filteredMeals,
    );

    emit(
      state.copyWith(status: HistoryStatus.success, historyData: filteredData),
    );
  }
}
